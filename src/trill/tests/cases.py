...
testcases: Sequence[TestCase] = [
    ...
    TestCase('(4+1)d6', 7, ['(d (group (+ 4 1)) 6)'],
             [[3.5, 3.5, 3.5, 3.5, 3.5]]),
    ...
    TestCase('10#(sum 5d6)', 8, [
             '(# 10 (group (sum (d 5 6))))'], [[5 * 3.5] * 10]),
    TestCase('3d6 U 3d8', 7, ['(U (d 3 6) (d 3 8))'],
             [[3.5, 3.5, 3.5, 4.5, 4.5, 4.5]]),
    TestCase('3d6;', 4, None, None,
    ...
    TestCase('{d6, 3d8}', 8, ['(collection (d 6) (d 3 8))'], [
             [3.5, 4.5, 4.5, 4.5]]),
    TestCase('{1..6}', 5, ['(collection (.. 1 6))'], [[1, 2, 3, 4, 5, 6]]),
    TestCase('choose {-1, 0, 1}', 9, ['(choose (collection (- 1) 0 1))'], [0]),
    TestCase('{1..10} pick 3', 7, [
             '(pick (collection (.. 1 10)) 3)'], [[5, 6, 7]]),
    TestCase('{1, 2, 3} pick 4', 9, [
             '(pick (collection 1 2 3) 4)'], [[1, 2, 3]]),
    TestCase('min {1, 2, 3}', 8, ['(min (collection 1 2 3))'], [1]),
    TestCase('max {1, 2, 3}', 8, ['(max (collection 1 2 3))'], [3]),
    TestCase('minimal {1, 1, 2, 3, 3}', 12, [
             '(minimal (collection 1 1 2 3 3))'], [[1, 1]]),
    TestCase('maximal {1, 1, 2, 3, 3}', 12, [
    ...
    TestCase('{1, 2, 3, 4} drop 3', 11, [
             '(drop (collection 1 2 3 4) 3)'], [[1, 2, 4]]),
    TestCase('d6 drop d6', 5, ['(drop (d 6) (d 6))'], [[]]),
    TestCase('{1, 2, 3, 4} keep 3', 11, [
             '(keep (collection 1 2 3 4) 3)'], [[3]]),
    TestCase('{2, 2, 3} drop {2, 4}', None, None, [[3]]),
    TestCase('d6 keep d6', 5, ['(keep (d 6) (d 6))'], [[3.5]]),
    TestCase('{2, 2, 3} -- {2, 4}', 13,
             ['(-- (collection 2 2 3) (collection 2 4))'], [[2, 3]]),
    TestCase('different {2, 1, 2}', 8, [
             '(different (collection 2 1 2))'], [[1, 2]]),
    TestCase('median {2, 6, 23}', 8, ['(median (collection 2 6 23))'], [6]),
    TestCase('x := d6; x*x', 8,
             ['(block (assign x (d 6)); (* x x))'], [3.5 * 3.5]),
    TestCase(
        'x := d6; y := d8; x*x*y*y',
        17,
    ...
    TestCase('x := 3d6; x', 7, [
             '(block (assign x (d 3 6)); x)'], [[3.5, 3.5, 3.5]]),
    TestCase(
    ...
    TestCase('foreach x in 1..3 do x+1', 10,
             ['(foreach x (.. 1 3) (+ x 1))'], [[2, 3, 4]]),
    TestCase('repeat x:=d8 until x<8', 9, [
             '(repeat until (assign x (d 8)) (< x 8))'], [4.5]),
    TestCase('repeat x:=d8 while x=8', 9, [
             '(repeat while (assign x (d 8)) (= x 8))'], [4.5]),
    TestCase(
    ...
    TestCase('N := 3; N d6', 7, [
             '(block (assign N 3); (d N 6))'], [[3.5, 3.5, 3.5]]),
    TestCase('N := 4; \\ This is a comment\nN d6', 7, [
             '(block (assign N 4); (d N 6))'], [[3.5, 3.5, 3.5, 3.5]]),
    TestCase('x := 3; x ~ 4', 7, ['(block (assign x 3); (~ x 4))'], [3]),
    TestCase('x ~ 4', 3, ['(~ x 4)'], [4]),
    TestCase('x := 6d6; [max x, count different x]', 14, [
             '(block (assign x (d 6 6)); (pair (max x) (count (different x))))'], [(3.5, 1)]),
    TestCase('p := [1, 2]; %1 p', 10, [
             '(block (assign p (pair 1 2)); (%1 p))'], [1]),
    TestCase('p := [1, 2]; %2 p', 10, [
             '(block (assign p (pair 1 2)); (%2 p))'], [2]),
    TestCase('x := {}; !x', 7, [
             '(block (assign x (collection )); (! x))'], [1]),
...
call down(10)""", 30, ['(function down (n) (block (assign x (d n)); (if (= x 1) 1 (+ x (call down x)))))', '(call down 10)'], None),
    TestCase("""sum largest 3 4d6""", 6, [
             "(sum (largest 3 (d 4 6)))"], [3 * 3.5]),
    TestCase("' sum largest 3 4d6", 7, [
             "(textbox 1 (sum (largest 3 (d 4 6))))"], [['10.5']]),
    TestCase("6 ' sum largest 3 4d6", 8, ["(textbox 6 (sum (largest 3 (d 4 6))))"], [
             ['10.5', '10.5', '10.5', '10.5', '10.5', '10.5']]),
    TestCase('"1|>two|>three"', 1, ["1|>two|>three"], ["1    \ntwo  \nthree"]),
    TestCase('"1" |> "two" |> "three"', 5, [
             "(textalign 1 (textalign two three))"], ["1    \ntwo  \nthree"]),
    TestCase('"1" <| "two" <| "three"', 5, [
             "(textalign 1 (textalign two three))"], ["    1\n  two\nthree"]),
    TestCase('"1" <> "two" <> "three"', 5, [
             "(textalign 1 (textalign two three))"], ["  1  \n two \nthree"]),
    TestCase('"1" || "two" <> "three"', 5, [
             "(textalign 1 (textalign two three))"], ["1 two \n three"]),
    TestCase(
