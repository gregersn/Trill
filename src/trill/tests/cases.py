...
testcases: Sequence[TestCase] = [
    ...
    TestCase(
        "{2, 2, 3} -- {2, 4}",
        13,
        ["(-- (collection 2 2 3) (collection 2 4))"],
        [[2, 3]],
    ),
    ...
    TestCase(
        "N := 4; \\ This is a comment\nN d6",
        7,
        ["(block (assign N 4); (d N 6))"],
        [[3.5, 3.5, 3.5, 3.5]],
    ),
    TestCase("x := 3; x ~ 4", 7, ["(block (assign x 3); (~ x 4))"], [3]),
    TestCase("x ~ 4", 3, ["(~ x 4)"], [4]),
    TestCase(
        "x := 6d6; [max x, count different x]",
        14,
        ["(block (assign x (d 6 6)); (pair (max x) (count (different x))))"],
        [(3.5, 1)],
    ),
    ...
    TestCase(
        "(min v)*call mul(largest ((count v)-1) v)",
        19,
        ["(* (group (min v)) (call mul (largest (group (- (group (count v)) 1)) v)))"],
        None,
    ),
    TestCase("""function foo(v) = v * v\ncall foo(5)""", None, None, [None, 25]),
    TestCase(
        """function foo(v) = if v then call foo(v - 1) + v else 0\ncall foo(3)""",
        None,
        None,
        [None, 6],
    ),
...
call mul(5d10)
    """,
        37,
        [
            "(function mul (v) (if v (* (group (min v)) (call mul (largest (group (- (group (count v)) 1)) v))) 1))",
            "(call mul (d 5 10))",
        ],
        [None, 5.5**5],
    ),
...
call down(10)""",
        30,
        [
            "(function down (n) (block (assign x (d n)); (if (= x 1) 1 (+ x (call down x)))))",
            "(call down 10)",
        ],
        None,
    ),
    ...
    TestCase(
        '"1" |> "two" |> "three"',
        5,
        ["(textalign 1 (textalign two three))"],
        ["1    \ntwo  \nthree"],
    ),
    TestCase(
        '"1" <| "two" <| "three"',
    ...
    TestCase(
        ...
        call add(1, 2)""",
        18,
        ["(function add (a,b) (+ a b))", "(call add 1 2)"],
        [None, 3],
    ),
    TestCase(
